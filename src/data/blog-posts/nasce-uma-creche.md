...
- Longas horas: passei a pessoalmente verificar 100% do código que era submetido à produção, logo minha carga de trabalho foi multiplicada pelo número de desenvolvedores da equipe;
...
Então, busquei formas de aumentar o tempo entre o código estar "funcionando" e ser enviado para revisão, utilizando automações e definindo padrões a serem seguidos.
...
Mas, apesar de padronizado, ainda havia uma forte dependência na capacidade dos desenvolvedores e minha de detectar erros que impediriam a execução do código. Descobri que havia a necessidade de um [*linter*](https://en.wikipedia.org/wiki/Lint_(software)), como diz a própria documentação do Prettier, em tradução livre, "use o Prettier para formatação e *linters* para capturar *bugs*!".
...
Uma ferramenta muito poderosa, permitindo não só a seleção dos arquivos que foram modificados no commit, como:

- executar validações diferentes dependendo da extensão ou nome do arquivo, utilizando expressões regulares;
- validação de arquivos parcialmente em stage, ou seja, quando existem mais modificações no arquivos que não foram selecionadas ao commit elas são empilhadas, as validações são executadas e as modificações são recuperadas automaticamente.
...
Agora temos garantia de um histórico com código bem formatado, mas e o histórico em si é legível? observar o log do git seria simples e traria informações úteis? Essa é a próxima pergunta que busquei respostas.
...
Configurado para os padrões de commits estabelecidos pela equipe do Angular, e adicionado o git hook `commit-msg`, aqui foi onde houve a maior barreira de adesão por parte dos desenvolvedores; a experiência me levou a buscar formas de simplificar a escrita de commits, semelhante a um tutorial.

### [Commitzen](https://commitizen-tools.github.io/commitizen/)

A salvação do onboarding, aqui escolhi não por o git hook por questões de compatibilidade com a interface de commits do VSCode, editor usado pela maioria da equipe; com as duas ferramentas era possível que os mais inexperientes no formato pudessem ser guiados a escrever uma mensagem que passasse na validação, enquanto os ousados pudessem escrever com velocidade e ser avisados em casos de erros.
...
Por usarmos Expo, o lançamento de atualizações era feito com um comando que demorava em torno de 8 minutos pra ser concluído e a atualização estar disponível para os clientes. Quando vi os fluxos de trabalho do GitHub, já visualizei o tempo que eu ganharia colocando deploys automáticos toda vez que o código da `main` era atualizado.
...
O panorama geral do projeto anterior a essas ferramentas podia ser descrito como nebuloso: não havia certeza além do que era testado manualmente e da qualidade garantida pela experiência dos envolvidos. Com o advento de ferramentas e processos, passou a existir uma documentação e garantias da evolução e qualidade do código escrito, criando alguns traumas e feedbacks engraçados no caminho, mas nenhuma história que não combinasse com o nome creche.

Ainda houveram mais dores e melhorias feitas ao longo do tempo a esse fluxo de trabalho, mas espero com esse artigo ter mostrado que não é necessário almejar a implantação da creche 3.0 para se ter um impacto positivo no seu dia a dia.
